...
class Expandable : FrameLayout {
    ...
    var isExpanded by Delegates.observable(false) { _, _, newValue ->
    ...
    private fun buildView() {
        ...
        if (headerView == null) headerView = getChildAt(0) ?: throw IllegalStateException("Add a HeaderView to your ExpandableView")
        if (expandableView == null) expandableView = getChildAt(1) ?: throw IllegalStateException("Add a ContentView to your ExpandableView")
    ...
    private fun creteHeader() {
        headerLayout.layoutParams = LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.WRAP_CONTENT)
        createHeaderContent()
        createHeaderIcon()
        createExpandIcon()
    ...
    private fun createHeaderIcon() {
        ...
        iconImage.apply {
            layoutParams = LayoutParams(iconSize, iconSize, Gravity.START or Gravity.TOP).apply { setMargins(padding) }
        }
    ...
    private fun createExpandIcon() {
        ...
        expandIcon.apply {
            layoutParams = LayoutParams(iconSize, iconSize, Gravity.END or Gravity.TOP).apply { setMargins(padding, padding, padding, 0) }
            setImageDrawable(if (expandIndicator == null) null else expandIndicator)
        }
    ...
    private fun createHeaderContent() {
        ...
        val endPadding = padding * 2

        val headerLayoutParams = headerView!!.layoutParams as LayoutParams
        headerView!!.setPadding(startPadding, 0, 0, 0)
    ...
    private fun createExpandableHeight() {
        ...
        expandableView?.measure(
            MeasureSpec.makeMeasureSpec(deviceWidth, MeasureSpec.AT_MOST), MeasureSpec.makeMeasureSpec(0, MeasureSpec.UNSPECIFIED)
        )
